...
- **استخدام `ApiResult` بشكل غير دقيق:** 
  - *الملف:* `app_date_repository.dart`
  - *المشكلة:* الـ Repository يُرجع `ApiResult` رغم أنه يتعامل مع `ILocalStorageService` (تخزين محلي) وليس مع شبكة (API). هذا يسبب التباساً وكان من الأفضل استخدام نوع إرجاع عام مثل `Either` أو `Result` للتخزين المحلي.
