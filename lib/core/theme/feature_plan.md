...
### 📌 المرحلة 1: دمج وتصفية الألوان في الملف الحالي (`app_colors.dart`)
- [ ] استبدال درجات `success` و `textSuccess` و `iconSuccess` بـ `secondry` في كامل الكود المصدري.
- [ ] استبدال `appCardGreen` بـ `secondry` أو `secondaryBackground` في الكروت المعنية.
- [ ] استبدال `warning` بـ `primary` أو درجات مناسبة في الكود.
- [ ] نقل لون الفيسبوك `facebookBlue` محلياً داخل واجهته وحذفه من المجلد المركزي.
- [ ] تنظيف ملف `app_colors.dart` الحالي ليحتوي فقط على الألوان التسعة الأساسية المعتمدة للظلام.
- [ ] فحص بناء التطبيق والتأكد البصري من مطابقة المظهر للنسخة السابقة تماماً.

### 📌 المرحلة 2: تجربة وضبط الألوان من قِبل المستخدم (User Testing & Tweaking)
- [ ] توقف المساعد الذكي بالكامل لإتاحة الفرصة للمستخدم لتعديل درجات الألوان ورؤية النتائج بصرياً في وضع الإضاءة وتثبيت لوحة الألوان النهائية.
