...
void setupQiblaDependencies(GetIt sl) {
  sl
    ..registerLazySingleton<QiblaLocalDataSource>(
      () => QiblaLocalDataSourceImpl(sl<LocalStorageService>()),
    )
    ..registerLazySingleton<QiblaService>(QiblaServiceImpl.new)
    ..registerLazySingleton<QiblaRepository>(
      () => QiblaRepoImpl(
        localDataSource: sl<QiblaLocalDataSource>(),
        qiblaService: sl<QiblaService>(),
      ),
    )
    ..registerLazySingleton<GetQiblaDirectionUseCase>(
      () => GetQiblaDirectionUseCase(sl<QiblaRepository>()),
    )
    ..registerLazySingleton<GetQiblaCompassStreamUseCase>(
      () => GetQiblaCompassStreamUseCase(
        service: sl<QiblaService>(),
        repository: sl<QiblaRepository>(),
      ),
    )
